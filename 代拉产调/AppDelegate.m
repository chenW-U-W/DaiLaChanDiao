...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    PendingViewController *mainVC = [[PendingViewController alloc] init];
...
#warning 测试 开发环境 时需要修改BPushMode为BPushModeDevelopment 需要修改Apikey为自己的Apikey
    ...
    [BPush registerChannel:launchOptions apiKey:@"FIskKPwd2WaCqvqrZXsNgrri" pushMode:BPushModeDevelopment withFirstAction:nil withSecondAction:nil withCategory:nil isDebug:YES];
    // App 是用户点击推送消息启动
...
- (void)application:(UIApplication *)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken
{
    ...
    [BPush bindChannelWithCompleteHandler:^(id result, NSError *error) {
        ...
        DLog(@"%@",[NSString stringWithFormat:@"Method: %@\n%@",BPushRequestMethodBind,result]);
    ...
    // 打印到日志 textView 中
    DLog(@"%@",[NSString stringWithFormat:@"Register use deviceToken : %@",deviceToken]);
